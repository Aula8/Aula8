...
public class FragmentChat extends android.support.v4.app.Fragment {
    ...
    private ImageButton botonArchivo;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        botonArchivo = (ImageButton) rootView.findViewById(R.id.attach_button);
        botonArchivo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new FileChooser((Activity) view.getContext()).setFileListener(new FileChooser.FileSelectedListener()
                {
                    @Override
                    public void fileSelected(final File file)
                    {
                        try
                        {
                            ...
                            String subject = Servidor.getActualSubject();
                            String section = Servidor.getActualSection();
                            FilesController.uploadFile(file, "FILES/", subject, section, getContext());
                        }
                        catch (Exception e)
                        {
                            e.printStackTrace();
